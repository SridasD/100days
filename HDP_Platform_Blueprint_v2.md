...
export async function writeAuditLog({
  userId, action, entity, entityId, req, meta
}: AuditParams) {
...
**Page:** `/admin/reports/district` + `/public/district/[id]`
...
### B.2 Screen 2 — Department Project List (`/public/department/[sec_id]`)

**Legacy URL:** `?action=category&sec_id=X` (public, no login)  
**New Route:** `/public/department/[sec_id]`
...
| Legacy | New |
|---|---|
| "Show ___ entries" dropdown | ShadCN `<Select>` for page size |
| "Search:" text input | ShadCN `<Input>` with debounced filter |
| Pagination (Previous / 1 / Next) | ShadCN `<Pagination>` |

---

### B.3 Screen 3 — Indicator Detail (`/public/department/[sec_id]/project/[project_id]`)

**Legacy URL:** `?action=project_details&project_id=X`  
**New Route:** `/public/department/[sec_id]/project/[project_id]`
...
### B.4 Screen 4 — Media Gallery (`/public/gallery/[project_id]`)

**Legacy URL:** `?action=gallery&project_id=X`  
**New Route:** `/public/gallery/[project_id]`
...
/ (Home Dashboard)
  ...
  ├── District-wise grid
  │     └── Click district → /public/district/[district_id]
  ├── Bottom summary bar
  └── Click department card → /public/department/[sec_id]
        ├── Project cards with progress (Dept vs Verified)
        │     └── "ഘടകങ്ങൾ കാണുക →" → /public/department/[sec_id]/project/[project_id]
        │           ├── Indicator detail cards (progress bars, district, beneficiaries)
        │           └── "ചിത്രങ്ങൾ/വീഡിയോകൾ →" → /public/gallery/[project_id]
...
| `/public/department/[sec_id]` | Department project list | `?action=category&sec_id=X` |
| `/public/department/[sec_id]/project/[project_id]` | Project indicator details | `?action=project_details&project_id=X` |
| `/public/gallery/[project_id]` | Photo/video gallery | `?action=gallery&project_id=X` |
| `/public/district/[district_id]` | District-wise view | New (not in legacy) |
...
*Employment Generation - Target (read-only, from project):*
| Field | Value |
|---|---|
...
*Employment Generation - Achievement (editable by verifier):*
| Field | Maps to |
|---|---|
