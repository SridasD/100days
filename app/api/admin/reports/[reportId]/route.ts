...
/**
 * Rows 1-4: the shared blue/white banner (title, subtitle, generated-on,
 * spacer) used by every department-wise-style report export. `lastColumn`
 * is the rightmost data column letter (e.g. "N") so the merge spans the
 * full table width regardless of how many columns a given report has.
 */
function buildReportBanner(
  sheet: ExcelJS.Worksheet,
  options: { subtitle: string; lastColumn: string },
) {
  const { subtitle, lastColumn } = options;
...
/**
 * Writes the dark-blue, filterable, frozen table header row at `rowIndex`
 * and freezes everything above and including it. Data rows must start at
 * `rowIndex + 1`.
 */
function buildTableHeaderRow(
  sheet: ExcelJS.Worksheet,
  options: { headerValues: string[]; lastColumn: string; rowIndex: number },
) {
  const { headerValues, lastColumn, rowIndex } = options;
  ...
  sheet.autoFilter = { from: `A${rowIndex}`, to: `${lastColumn}${rowIndex}` };
...
function buildDepartmentWiseWorkbook(rows: ReportRow[]) {
  ...
  sheet.columns = [
    ...
    { header: "Districts", key: "districts", width: 24 },
    { header: "Total No. of Indicators", key: "noIndicators", width: 16 },
    { header: "Marked as Completed by Department", key: "completedByDepartment", width: 18 },
    { header: "Total Indicators Verified by Verifier", key: "verifiedByVerifier", width: 18 },
    { header: "Indicator marked as Completed by Verifier", key: "completedByVerifier", width: 18 },
  ...
  buildReportBanner(sheet, {
    subtitle: "Department-wise Project Report",
    lastColumn: "N",
  });
  buildTableHeaderRow(sheet, {
    headerValues: [
      "Sl. No",
      "Administrative Department",
      ...
      "Marked as Completed by Department",
      "Total Indicators Verified by Verifier",
      "Indicator marked as Completed by Verifier",
      "Project Status",
      "Project Outlay",
      "Source of Funding",
      "Nature of Project",
  ...
  sheet.addRows(
    rows.map((row, index) => ({
      ...
      completedByDepartment: toNumber(row.completed_by_department),
      verifiedByVerifier: toNumber(row.verified_by_verifier),
      completedByVerifier: toNumber(row.completed_by_verifier),
  ...
  for (let rowIndex = dataStartRow; rowIndex <= dataEndRow; rowIndex += 1) {
    ...
    row.eachCell({ includeEmpty: true }, (cell, columnNumber) => {
      ...
      cell.alignment = {
        vertical: "middle",
        wrapText: true,
        horizontal: [1, 7, 8, 9, 10, 11].includes(columnNumber) ? "center" : "left",
      };
      if (rowIndex % 2 === 0) {
        cell.fill = {
          type: "pattern",
  ...
  sheet.getColumn(12).alignment = { horizontal: "right" };
  sheet.getColumn(12).numFmt = '"Rs" #,##0.00';
...
async function generateDepartmentWiseReportRows() {
  const result = await db.execute(sql`
    SELECT
      ...
      COALESCE(mp.project_name, 'Untitled project') AS project_name,
      COALESCE(
        STRING_AGG(DISTINCT mdist.district_name, ', ' ORDER BY mdist.district_name),
        'Unassigned'
      ) AS districts,
      COUNT(DISTINCT i.indicator_id)::int AS no_indicators,
      COUNT(DISTINCT CASE WHEN i.completed_date IS NOT NULL THEN i.indicator_id END)::int AS completed_by_department,
      COUNT(DISTINCT CASE WHEN i.verified_date IS NOT NULL THEN i.indicator_id END)::int AS verified_by_verifier,
      COUNT(DISTINCT CASE WHEN i.completed_date IS NOT NULL AND i.verified_date IS NOT NULL THEN i.indicator_id END)::int AS completed_by_verifier,
      CASE
    ...
    LEFT JOIN hdp.master_district mdist ON mdist.district_id = i.district_id
...
async function generateDepartmentWiseReportCSV() {
  ...
  const csvRows = rows.map((row, index) => [
    ...
    Number(row.completed_by_department ?? 0),
    Number(row.verified_by_verifier ?? 0),
    Number(row.completed_by_verifier ?? 0),
...
type DeptAverageRow = {
  ...
  num_projects: number;
  verified_completed_projects: number;
  total_indicators: number;
  verified_completed_indicators: number;
  dept_physical_avg: number | null;
};

type DeptAverageEnriched = DeptAverageRow & {
  diff: number | null;
  comparison: string;
};
...
/**
 * Department rollup for the State Average Report. Physical progress
 * matches physicalRollup() in components/reports/tabular/lib.ts (the
 * formula behind /admin/osd/project-performance-dashboard) so the two
 * reports reconcile: per indicator, unverified rows are zero-filled
 * (COALESCE(verified_percentage, 0)), not excluded; per project, the
 * average is taken across that project's indicators (a project with zero
 * indicators counts as 0%, via project_physical.project_physical_avg
 * defaulting to 0 rather than NULL); per department, the average is taken
 * across its projects' own averages — project-weighted, not
 * indicator-weighted.
 */
async function generateStateAverageReportRows(): Promise<DeptAverageRow[]> {
  const result = await db.execute(sql`
    WITH dept_projects AS (
      SELECT DISTINCT ms.sec_id, ms.secretary_name, mp.project_id, mp.is_completed
    ...
    project_physical AS (
      -- One row per project — must stay 1:1 per project so joining it
      -- below can't fan out against the (1:N) indicators join.
      SELECT
        dp.sec_id,
        dp.project_id,
        AVG(COALESCE(i.verified_percentage, 0)) AS project_physical_avg
      FROM dept_projects dp
      LEFT JOIN hdp.indicators i ON i.project_id = dp.project_id
      GROUP BY dp.sec_id, dp.project_id
    ),
    dept_physical AS (
      -- One row per department — pre-aggregated separately so this join
      -- doesn't get multiplied by the indicators join in the final SELECT.
      SELECT sec_id, AVG(project_physical_avg) AS dept_physical_avg
      FROM project_physical
      GROUP BY sec_id
    )
    SELECT
      dp.sec_id,
      dp.secretary_name,
      COUNT(DISTINCT dp.project_id)::int AS num_projects,
      COUNT(DISTINCT CASE WHEN dp.is_completed = 2 THEN dp.project_id END)::int AS verified_completed_projects,
      COUNT(DISTINCT i.indicator_id)::int AS total_indicators,
      COUNT(DISTINCT CASE
        WHEN i.verified_date IS NOT NULL AND COALESCE(i.verified_percentage, 0) >= 100
        THEN i.indicator_id END)::int AS verified_completed_indicators,
      MAX(dph.dept_physical_avg)::numeric(5,2) AS dept_physical_avg
    FROM dept_projects dp
    LEFT JOIN hdp.indicators i ON i.project_id = dp.project_id
    LEFT JOIN dept_physical dph ON dph.sec_id = dp.sec_id
    GROUP BY dp.sec_id, dp.secretary_name
    ORDER BY dp.secretary_name
  `);

  return result.rows.map((r) => {
    const row = r as Record<string, unknown>;
    return {
      sec_id: Number(row.sec_id),
      secretary_name: String(row.secretary_name ?? "Unassigned"),
      num_projects: toNumber(row.num_projects),
      verified_completed_projects: toNumber(row.verified_completed_projects),
      total_indicators: toNumber(row.total_indicators),
      verified_completed_indicators: toNumber(row.verified_completed_indicators),
      dept_physical_avg:
        row.dept_physical_avg == null ? null : Number(row.dept_physical_avg),
    };
  });
}

/**
 * State average is department-weighted: the mean of each department's own
 * physical-progress average (from generateStateAverageReportRows, itself
 * project-weighted), not a straight average across every indicator or
 * project statewide. dept_physical_avg is only ever null for a department
 * with zero projects — which can't happen here since dept_projects is an
 * inner join — so this null branch is a defensive fallback, not the
 * common case (unlike before this was aligned to physicalRollup()).
 */
function computeStateAverageSummary(rows: DeptAverageRow[]): StateAverageSummary {
  const COMPARISON_EPSILON = 0.005;
  const withProgress = rows.filter(
    (r): r is DeptAverageRow & { dept_physical_avg: number } =>
      r.dept_physical_avg !== null,
  );

  const stateAverage = withProgress.length
    ? withProgress.reduce((sum, r) => sum + r.dept_physical_avg, 0) /
      withProgress.length
    : null;

  const enriched: DeptAverageEnriched[] = rows.map((r) => {
    if (r.dept_physical_avg === null || stateAverage === null) {
      return { ...r, diff: null, comparison: "N/A" };
    }
    const diff = r.dept_physical_avg - stateAverage;
    const comparison =
      Math.abs(diff) < COMPARISON_EPSILON
        ? "Equal to State Average"
        : diff > 0
          ? "Above State Average"
          : "Below State Average";
    return { ...r, diff, comparison };
  });

  const rankable = enriched.filter(
  ...
  return {
    ...
    aboveCount: enriched.filter((r) => r.comparison === "Above State Average").length,
    belowCount: enriched.filter((r) => r.comparison === "Below State Average").length,
    highest,
    lowest,
  };
}

// Whole-number percentages, matching roundPct() in
// components/reports/tabular/lib.ts — the dashboard never shows decimals.
function formatPct(value: number | null) {
  return value === null ? "N/A" : `${Math.round(value)}%`;
}

function formatDiffPct(value: number | null) {
  if (value === null) return "N/A";
  const rounded = Math.round(value);
  return `${rounded >= 0 ? "+" : ""}${rounded}%`;
}

const STATE_AVERAGE_TABLE_HEADERS = [
  "Sl. No",
  "Department Name",
  "No. of Projects",
  "Total Indicator Count",
  "No. of Verified Completed Projects",
  "No. of Verified Completed Indicators",
  "Department Physical Progress Average (%)",
...
/**
 * A clean two-column Metric/Value table on its own sheet, styled like the
 * department table's own header row for visual consistency.
 */
function buildStateAverageSummarySheet(
  workbook: ExcelJS.Workbook,
  summary: StateAverageSummary,
) {
  ...
  buildReportBanner(sheet, {
    subtitle: "State Average Report — Summary",
    lastColumn: "B",
  });

  sheet.mergeCells("A5:B5");
  sheet.getCell("A5").value = "Applied Filters: None";
  sheet.getCell("A5").font = { size: 10, italic: true, color: { argb: "FF6B7280" } };
  sheet.getCell("A5").alignment = { horizontal: "center", vertical: "middle" };

  const tableHeaderRow = 7;
  buildTableHeaderRow(sheet, {
    headerValues: ["Metric", "Value"],
    lastColumn: "B",
    rowIndex: tableHeaderRow,
  });

  const summaryLines = summaryMetricLines(summary);
  const dataStartRow = tableHeaderRow + 1;
  sheet.addRows(summaryLines.map(([metric, value]) => ({ metric, value })));
...
function buildStateAverageDetailSheet(
  workbook: ExcelJS.Workbook,
  summary: StateAverageSummary,
) {
  const sheet = workbook.addWorksheet("Department-wise Details");
  sheet.columns = [
    { key: "slNo", width: 8 },
    { key: "departmentName", width: 30 },
    { key: "numProjects", width: 14 },
    { key: "totalIndicators", width: 16 },
    { key: "verifiedCompletedProjects", width: 16 },
    { key: "verifiedCompletedIndicators", width: 16 },
    { key: "physicalAvg", width: 18 },
    { key: "comparison", width: 20 },
    { key: "diff", width: 18 },
  ];

  buildReportBanner(sheet, {
    subtitle: "State Average Report — Department-wise Details",
    lastColumn: "I",
  });
  ...
  const tableHeaderRow = 7;
  buildTableHeaderRow(sheet, {
    headerValues: STATE_AVERAGE_TABLE_HEADERS,
    lastColumn: "I",
    rowIndex: tableHeaderRow,
  });
  ...
  sheet.addRows(
    summary.rows.map((row, index) => ({
      slNo: index + 1,
      departmentName: row.secretary_name,
      numProjects: row.num_projects,
      totalIndicators: row.total_indicators,
      verifiedCompletedProjects: row.verified_completed_projects,
      verifiedCompletedIndicators: row.verified_completed_indicators,
      physicalAvg: formatPct(row.dept_physical_avg),
      comparison: row.comparison,
      diff: formatDiffPct(row.diff),
    })),
  );

  const dataEndRow = dataStartRow + summary.rows.length - 1;
  for (let rowIndex = dataStartRow; rowIndex <= dataEndRow; rowIndex += 1) {
    const row = sheet.getRow(rowIndex);
    row.height = 20;
    row.eachCell({ includeEmpty: true }, (cell, columnNumber) => {
...
function buildStateAverageWorkbook(summary: StateAverageSummary) {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = "HDP Portal";
  workbook.company = "Kerala CMO";
  workbook.created = new Date();
  ...
  buildStateAverageSummarySheet(workbook, summary);
  buildStateAverageDetailSheet(workbook, summary);

  return workbook;
}

/**
 * CSV export for the state average report: a flattened Metric,Value
 * summary block, a blank separator line, then the department-wise table.
 */
function generateStateAverageReportCSV(summary: StateAverageSummary) {
  const summaryHeaders = ["Metric", "Value"];
  const summaryRows = summaryMetricLines(summary);

  const deptRows = summary.rows.map((row, index) => [
    index + 1,
    row.secretary_name,
    row.num_projects,
    row.total_indicators,
    row.verified_completed_projects,
    row.verified_completed_indicators,
    formatPct(row.dept_physical_avg),
    row.comparison,
    formatDiffPct(row.diff),
  ]);
  ...
  return [
    ...
    "",
    STATE_AVERAGE_TABLE_HEADERS.map(csvEscape).join(","),
    ...deptRows.map((r) => r.map(csvEscape).join(",")),
  ].join("\n");
}
  ...
  try {
    ...
    switch (reportId) {
      ...
      case "department":
        ...
        break;
      case "state-average": {
        const summary = computeStateAverageSummary(
          await generateStateAverageReportRows(),
        );
        if (format === "xlsx") {
          const workbook = buildStateAverageWorkbook(summary);
          const buffer = (await workbook.xlsx.writeBuffer()) as ArrayBuffer;
          const timestamp = formatFileTimestamp();
          return new NextResponse(Buffer.from(buffer), {
            headers: {
              "content-type":
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
              "content-disposition": `attachment; filename="HDP-State Average Report ${timestamp}.xlsx"`,
            },
          });
        }
        csv = generateStateAverageReportCSV(summary);
        break;
      }
