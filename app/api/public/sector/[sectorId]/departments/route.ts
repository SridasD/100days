    ...
    const r = await db.execute(sql`
      SELECT
        ...
        COALESCE((
          ...
          WHERE ps.sec_id = ms.sec_id AND mp.sector_id = ${id}
        ), 0) AS projects,
        ...
        COALESCE((
          ...
          INNER JOIN hdp.master_projects mp ON i.project_id = mp.project_id
          INNER JOIN hdp.project_secretary ps ON mp.project_id = ps.project_id
          WHERE ps.sec_id = ms.sec_id AND mp.sector_id = ${id} AND g.gallery_type = 1
        ), 0) AS image_count,
        ...
        COALESCE((
          ...
          INNER JOIN hdp.master_projects mp ON i.project_id = mp.project_id
          INNER JOIN hdp.project_secretary ps ON mp.project_id = ps.project_id
          WHERE ps.sec_id = ms.sec_id AND mp.sector_id = ${id} AND g.gallery_type = 2
        ), 0) AS video_count
